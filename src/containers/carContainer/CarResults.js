  ...
  useEffect(() => {
    data.forEach((car, index) => {
      if (car["Vendor"]["@Name"] === "ALAMO") {
        car["VehAvails"].map((p) =>
          Object.assign(p.Vehicle, { Vendor: "ALAMO" })
        );
      } else if (car["Vendor"]["@Name"] === "AVIS") {
        car["VehAvails"].map((p) =>
          Object.assign(p.Vehicle, { Vendor: "AVIS" })
        );
      } else if (car["Vendor"]["@Name"] === "HERTZ") {
        car["VehAvails"].map((p) =>
          Object.assign(p.Vehicle, { Vendor: "HERTZ" })
        );
      }
      ...
      <div className="info">
        <div className="info_header">
              ...
              <table>
                ...
                {selectedItems.map((el, id) => (
                  <tr key={id}>
                    ...
                    <td>
                      <button onClick={() => removeFromCompare(el)}>
                        Remove
                      </button>
                    </td>
                  </tr>
          ...
          <div className="sort">
            <p>Sort by</p>
            <div>
              <button onClick={() => setFilterVal(false)}>
                Price low to high
              </button>
              |
              <button onClick={() => setFilterVal(true)}>
                Price high to low
              </button>
            </div>
          </div>
      ...
      <div className="results">
        <ul className="results_listing">
          {!filterVal &&
            carArray
              .sort((a, b) => {
                return a["TotalCharge"]["@RateTotalAmount"].localeCompare(
                  b["TotalCharge"]["@RateTotalAmount"],
                  undefined,
                  { numeric: true }
                );
              })
          ...
          {filterVal &&
            carArray
              .sort((a, b) => {
                return b["TotalCharge"]["@RateTotalAmount"].localeCompare(
